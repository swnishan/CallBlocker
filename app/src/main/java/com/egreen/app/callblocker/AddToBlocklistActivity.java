...
public class AddToBlocklistActivity extends ActionBarActivity implements OnClickListener, AdapterView.OnItemClickListener {

    private static final String TAG = AddToBlocklistActivity.class.getName();
    ...
    private BootstrapButton reset_btn, submit_btn;
    ...
    private com.egreen.app.callblocker.Blacklist blackListNo;

    // Declaration of BlacklistDAO to interact with SQlite database
    private com.egreen.app.callblocker.BlacklistDAO blackListDao;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        blackListDao = new com.egreen.app.callblocker.BlacklistDAO(this);
        blackListNo = new com.egreen.app.callblocker.Blacklist();
        ...
        reset_btn = (BootstrapButton) findViewById(R.id.reset_btn);
        submit_btn = (BootstrapButton) findViewById(R.id.submit_btn);
        ...
        loadContactList();
        ArrayAdapter contactAdapter = new ArrayAdapter(AddToBlocklistActivity.this, android.R.layout.simple_list_item_1, listContact);
        phone_auto.setAdapter(contactAdapter);
        phone_auto.setOnItemClickListener(this);
    }

    private void loadCountries() {
        for (int i = 0; i < com.egreen.app.callblocker.CountryCodes.m_Codes.length; i++) {
            contryWithCodes.add(new ContryWithCode(com.egreen.app.callblocker.CountryCodes.m_Countries[i], com.egreen.app.callblocker.CountryCodes.m_Codes[i]));
        }
    }
...
//            e.printStackTrace();
            ...
            if (country_code_auto.getText().toString().trim().length() > 0 && phone_auto.getText().toString().trim().length() > 0) {
                ...
                if (phoneNo.length() > 9) {
                    for (ContryWithCode contryWithCode : contryWithCodes) {
                        if (phoneNo.startsWith("+" + contryWithCode.code)) {
                            phoneNo = phoneNo.replace("+" + contryWithCode.code, "");
                            break;
                        }
                        if (phoneNo.startsWith(contryWithCode.code)) {
                            phoneNo = phoneNo.replace(contryWithCode.code, "");
                            System.out.println("country code match " + contryWithCode.code);
                            break;
                        }
                    }
                }
                ...
                int isHave = blackListDao.searchPhoneNo(blackListNo.phoneNumber);

                if (isHave > 0) {
                    showMessageDialog("This number already blocked...");
                } else {
                    // Insert the object to the database
                    blackListDao.create(blackListNo);
    ...
    private void showDialog() {
        ...
        alertDialogBuilder.setMessage("Block number successfully...");

        // Add a positive button and it's action. In our case action would be, just hide the dialog box ,
        // and erase the user inputs.
        alertDialogBuilder.setPositiveButton("Add More", new DialogInterface.OnClickListener() {
